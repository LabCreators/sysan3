...
class PolynomialBuilder(object):
    def __init__(self, solution):
        ...
        self.modeltype = solution.modeltype
        self.predlength = solution.predlength
        self.poldegree = '2' if np.max(solution.poldegree) <= 3 else '5'
        self.bort_net = self.parse("libs/" + self.data + "/" + self.modeltype + "/" + self.predlength + "/" + self.poldegree + "/0.cab")
        self.fuel = self.parse("libs/" + self.data + "/" + self.modeltype + "/" + self.predlength + "/" + self.poldegree + "/1.cab")
        self.battery = self.parse("libs/" + self.data + "/" + self.modeltype + "/" + self.predlength + "/" + self.poldegree + "/2.cab")
        self.table = self.parsetable("libs/" + self.data + "/" + self.modeltype + "/" + self.predlength + "/" + self.poldegree + "/t.class")
        self.current_time = 0
    ...
    def plot_in_realtime(self):
        ...
        def animate_bort_net(i):
            ...
            time.setText(self.table[tableIndex][0])
            bort_net_val = QTableWidgetItem()
            bort_net_val.setText(self.table[tableIndex][1])
            fuel_val = QTableWidgetItem()
            fuel_val.setText(self.table[tableIndex][2])
            battery_val = QTableWidgetItem()
            battery_val.setText(self.table[tableIndex][3])
            state = QTableWidgetItem()
            state.setText(self.table[tableIndex][4])
            state = QTableWidgetItem()
            state.setText(self.table[tableIndex][4])
            state = QTableWidgetItem()
            state.setText(self.table[tableIndex][4])
            risk = QTableWidgetItem()
            risk.setText(self.table[tableIndex][5])
            cause = QTableWidgetItem()
            cause.setText(self.table[tableIndex][6])
            level = QTableWidgetItem()
            level.setText(self.table[tableIndex][7])
            resource = QTableWidgetItem()
            resource.setText(self.table[tableIndex][8])

            self.resultsField.setItem(tableIndex, 0, time)
            self.resultsField.setItem(tableIndex, 1, bort_net_val)
            self.resultsField.setItem(tableIndex, 2, fuel_val)
            self.resultsField.setItem(tableIndex, 3, battery_val)
            self.resultsField.setItem(tableIndex, 4, state)
            self.resultsField.setItem(tableIndex, 5, risk)
            self.resultsField.setItem(tableIndex, 6, cause)
            self.resultsField.setItem(tableIndex, 7, level)
            self.resultsField.setItem(tableIndex, 8, resource)

            time1 = QTableWidgetItem()
            time1.setText(self.table[tableIndex + 1][0])
            bort_net_val1 = QTableWidgetItem()
            bort_net_val1.setText(self.table[tableIndex + 1][1])
            fuel_val1 = QTableWidgetItem()
            fuel_val1.setText(self.table[tableIndex + 1][2])
            battery_val1 = QTableWidgetItem()
            battery_val1.setText(self.table[tableIndex + 1][3])
            state1 = QTableWidgetItem()
            state1.setText(self.table[tableIndex + 1][4])
            state1 = QTableWidgetItem()
            state1.setText(self.table[tableIndex + 1][4])
            state1 = QTableWidgetItem()
            state1.setText(self.table[tableIndex + 1][4])
            risk1 = QTableWidgetItem()
            risk1.setText(self.table[tableIndex + 1][5])
            cause1 = QTableWidgetItem()
            cause1.setText(self.table[tableIndex + 1][6])
            level1 = QTableWidgetItem()
            level1.setText(self.table[tableIndex + 1][7])
            resource1 = QTableWidgetItem()
            resource1.setText(self.table[tableIndex + 1][8])

            self.resultsField.setItem(tableIndex + 1, 0, time1)
            self.resultsField.setItem(tableIndex + 1, 1, bort_net_val1)
            self.resultsField.setItem(tableIndex + 1, 2, fuel_val1)
            self.resultsField.setItem(tableIndex + 1, 3, battery_val1)
            self.resultsField.setItem(tableIndex + 1, 4, state1)
            self.resultsField.setItem(tableIndex + 1, 5, risk1)
            self.resultsField.setItem(tableIndex + 1, 6, cause1)
            self.resultsField.setItem(tableIndex + 1, 7, level1)
            self.resultsField.setItem(tableIndex + 1, 8, resource1)
            self.resultsField.resizeColumnsToContents()

            if i >= len(self.bort_net):
